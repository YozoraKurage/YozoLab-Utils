...
namespace YozoLab.MeshBaker
{
    ...
    public class MeshBakeGroupEditor : Editor
    {
        ...
        {
            var bakeGroup = (MeshBakeGroup)target;
            ...
            while (iterator.NextVisible(enterChildren))
            {
                ...
                if (iterator.propertyPath == "rendererGroups")
                {
                    ...
                    DrawChildGroupSummary(bakeGroup);
                    DrawAutoAnalyzeDropArea(bakeGroup);
                    continue;
                }
                // Custom以外のモードでは収集プロパティは自動なので手動指定欄は隠す
                if (iterator.propertyPath == "textureProperties")
                {
                    ...
                    if (profile != null)
                    {
                        EditorGUILayout.HelpBox(
                            $"{bakeGroup.materialMode}モードでは収集テクスチャを自動決定します:\n" +
                            string.Join(", ", profile.properties),
                            MessageType.Info);
                        continue;
                    }
                }
            ...
            if (textureEntries != null)
            {
                ...
                {
                    DrawTextureEntry(bakeGroup, entry);
                }

                if (materialsWithoutTexture != null && materialsWithoutTexture.Count > 0)
                {
                    DrawNoTextureGroup(bakeGroup);
                }
            }

            EditorGUILayout.Space(12);

            bool hasRenderers = bakeGroup.GetEffectiveGroups()
            ...
            {
                EditorGUILayout.HelpBox(
                    "Renderer Groupsまたは子のMesh Bake Itemに、ベイク対象のRenderer" +
        ...
        /// <summary>子のMeshBakeItemコンポーネントの一覧を表示する</summary>
        private void DrawChildGroupSummary(MeshBakeGroup bakeGroup)
        {
            MeshBakeItem[] childGroups = bakeGroup.GetComponentsInChildren<MeshBakeItem>(true);
            if (childGroups.Length == 0) return;
            EditorGUILayout.HelpBox(
                $"子のMesh Bake Item ({childGroups.Length}): " +
        ...
        /// 子のMeshBakeItemとして生成するドロップ領域。
        /// </summary>
        private void DrawAutoAnalyzeDropArea(MeshBakeGroup bakeGroup)
        {
            Rect dropRect = GUILayoutUtility.GetRect(0, 44, GUILayout.ExpandWidth(true));
            GUI.Box(dropRect,
                "ここにオブジェクトをドロップで自動解析してグループ分け\n" +
                "（子としてMesh Bake Itemを生成します。オブジェクト自体は移動しません）",
            ...
            var claimed = new HashSet<Renderer>(
                bakeGroup.GetEffectiveGroups().SelectMany(g => g.renderers));
            ...
            List<RendererGroupAnalyzer.Proposal> proposals = RendererGroupAnalyzer.Analyze(
                targets,
                splitByMaterialSet: !bakeGroup.mergeMaterials,
            ...
            foreach (RendererGroupAnalyzer.Proposal proposal in proposals)
            {
                ...
                groupObject.transform.SetParent(bakeGroup.transform, false);

                MeshBakeItem group = groupObject.AddComponent<MeshBakeItem>();
        ...
        private void DrawTextureEntry(MeshBakeGroup bakeGroup, TextureEntry entry)
        {
            using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                using (new EditorGUILayout.VerticalScope())
                {
                    ...
                    DrawResolutionPopup(bakeGroup, entry);
        ...
        private void DrawNoTextureGroup(MeshBakeGroup bakeGroup)
        {
            using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
            {
        ...
        private void DrawResolutionPopup(MeshBakeGroup bakeGroup, TextureEntry entry)
        {
            int current = bakeGroup.GetTargetResolution(entry.texture);
            ...
            if (newValue != current)
            {
                SetTargetResolution(bakeGroup, entry.texture, newValue);
            }
        }

        /// <summary>解像度上書きをコンポーネントに非破壊で書き込む（Undo対応）</summary>
        private static void SetTargetResolution(MeshBakeGroup bakeGroup, Texture texture, int value)
        {
            Undo.RecordObject(bakeGroup, "Set Texture Resolution");

            TextureResolutionSetting setting =
                bakeGroup.textureResolutionSettings.FirstOrDefault(s => s != null && s.texture == texture);

            if (value == 0)
            {
                // 原寸（上書きなし）はエントリ自体を削除して設定を残さない
                if (setting != null) bakeGroup.textureResolutionSettings.Remove(setting);
            }
            ...
            {
                bakeGroup.textureResolutionSettings.Add(new TextureResolutionSetting
                {
                    texture = texture,
            ...
            EditorUtility.SetDirty(bakeGroup);
        }
        ...
        private void RunBake(MeshBakeGroup bakeGroup)
        {
            try
            {
                EditorUtility.DisplayProgressBar("Mesh Baker", "ベイク中...", 0.5f);
                BakeReport report = StaticMeshBaker.Bake(bakeGroup);
        ...
        private void InspectTextures(MeshBakeGroup bakeGroup)
        {
            ...
            string mainProperty = MaterialModeProfiles.GetCollectProperties(
                bakeGroup.materialMode, bakeGroup.textureProperties)[0];

            // 使用中のユニークなマテリアルを収集（配列グループ・子のMesh Bake Item両方から）
            var materials = new List<Material>();
            IEnumerable<Renderer> allRenderers = bakeGroup.GetEffectiveGroups()
